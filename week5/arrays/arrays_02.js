...
const myAlphabetLength = (array) => {
    array.length >= 5 ? console.log("Array length is sufficient") : console.log("Array length is too short.");
}
...
planets.forEach((planet, index) => console.log(`Planet: ${planet}, index: ${index}`));
...
mixedValues.forEach(i => console.log(`Item: ${i}, Type: ${typeof i}`));
...
reserve1Animals.forEach(animal1 => {
    reserve2Animals.forEach(animal2 => {
        if (animal1 === animal2) {
            console.log(`Common species: ${animal2}`)
        }
    })
})
...
animalNames.forEach(name => console.log(`Letters in ${name}: ${[...name].join(', ')}`));
...
const getPositiveTemperatures = (arr) => {
    let positiveTemperatures = arr.filter(i => i > 0);
...
penguinIndex > 0 ? console.log(penguinIndex) : console.log("Penguin is not in the zoo.");
...
const indexOfGiraffe = zooAnimals.indexOf('Giraffe');
...
console.log('sorted animals are', safariAnimals.sort());
...
console.log('reversed animals are', safariAnimals.reverse());
...
const johnIndexes = attendees.reduce((acc, person, index) => {
...
const uppercasedAttendies = attendees.map(a => a.toUpperCase());
console.log(uppercasedAttendies);
...
const newArr = [...forestAnimals, ...farmAnimals];
console.log(newArr);
...
numbers.splice(indexOfThirty, 2, 25, 27);
console.log(numbers);
...
let increasedPrices = prices.map(p => p + (p / 100 * 10));
